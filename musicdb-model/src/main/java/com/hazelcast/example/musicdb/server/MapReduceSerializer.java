...
public class MapReduceSerializer implements StreamSerializer<CustomSerializable> {

    private final ConcurrentMap<Bundle, List<MapReduceJobSerializer>> bundleSerializers = new ConcurrentHashMap<>();
    ...
    public void registerBundleSerializers(Bundle bundle, List<MapReduceJobSerializer> serializers) {
        bundleSerializers.put(bundle, serializers);
    ...
    public void unregisterBundleSerializers(Bundle bundle) {
        List<MapReduceJobSerializer> serializers = bundleSerializers.remove(bundle);
        if (serializers != null) {
            for (MapReduceJobSerializer serializer : serializers) {
                String className = serializer.serializationType().getCanonicalName();
                this.serializers.remove(className);
            }
        }
    }
